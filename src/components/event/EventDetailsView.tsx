...
export default function EventDetailsView({ event, isAdminView = false }: EventDetailsViewProps) {
    ...
    return (
        <div className="min-h-screen bg-[#F5F5F5] text-gray-800">
            ...
            {/* Hero Section */}
            <div className="relative w-full md:min-h-[520px] bg-gray-950 overflow-hidden flex items-center">
                ...
                <div className="absolute inset-0 select-none">
                    {event.detailImage || event.featureImage ? (
                        <Image
                            ...
                            fill
                            className="object-cover opacity-40 scale-105 transition-all duration-700"
                            alt=""
                            priority
                        />
                    ) : null}
                ...
                {/* Hero Container */}
                <div className="relative max-w-6xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center md:items-center gap-10 z-10 w-full text-white">
                    {/* Left: Poster (Thumbnail) */}
                    <div className="w-64 md:w-80 aspect-[2/3] relative rounded-2xl overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex-shrink-0 border border-white/20 group">
                        <Image
                            src={event.featureImage || event.detailImage || ''}
                            fill
                    ...
                    <div className="flex-1 space-y-6 text-center md:text-left">
                        <h1 className="text-3xl md:text-6xl font-black tracking-tighter leading-[1.1] drop-shadow-lg">{event.title}</h1>
                        ...
                        {/* Price */}
                        <div className="text-2xl font-black text-[#7030ef]">
                            {(() => {
                                if (event.entry) {
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <section className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4 border-b pb-2">
                            ...
                            {event.location && (
                                <a
                                    href={`https://www.google.com/maps/dir/?api=1&destination=${event.locationCoords?.coordinates ? `${event.locationCoords.coordinates[1]},${event.locationCoords.coordinates[0]}` : encodeURIComponent(event.location)}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-2 text-xs font-black text-[#7030ef] uppercase tracking-widest hover:underline"
                                >
                                    <Map size={14} />
                                    Get Directions
                                </a>
                        ...
                        <div className="space-y-4">
                            ...
                            {event.location && (
                                <div className="flex items-start gap-2 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100">
                                    <MapPin size={16} className="text-[#7030ef] flex-shrink-0 mt-0.5" />
