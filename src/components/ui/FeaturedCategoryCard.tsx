...
const FeaturedCategoryCard: React.FC<FeaturedCategoryCardProps> = ({ item, onClick }) => {
    return (
        <button
            onClick={onClick}
            className="group block relative aspect-square md:aspect-[4/5] overflow-hidden rounded-2xl shadow-card hover:shadow-premium transition-all duration-300 text-left w-full"
        >
            <Image
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-white/80 text-[10px] md:text-xs font-bold uppercase tracking-widest mt-2 bg-black/20 px-3 py-1 rounded-full backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
                    Explore Shows
                </p>
            </div>
